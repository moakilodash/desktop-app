...
export const Component = () => {
  const { t } = useAppTranslation('createUtxos')
  ...
  const onSubmit = (data: FormFields) => {
    ...
    createutxos({
      ...
      setIsLoading(false)
      if (res.error) {
        toast.error(t('notifications.error', { message: res.error.data.error }))
      } else {
        navigate(-1)
        ...
        <div className="space-y-6">
          <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6">
            ...
            <input
              ...
              max={10}
              min={1}
              placeholder={t('form.numberOfUtxos.placeholder')}
              type="number"
            />
          </div>
          ...
          <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6">
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                min={0}
                placeholder={t('form.utxoSize.placeholder')}
                type="number"
              />
              <span className="text-lg font-semibold text-white min-w-[100px] text-right">
                {size.toLocaleString()} {t('form.utxoSize.satoshis')}
              </span>
            </div>
            <input
          ...
          <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6">
            ...
            >
              <option value={feeRates.slow}>
                {t('form.feeRate.options.slow', { rate: feeRates.slow })}
              </option>
              <option value={feeRates.normal}>
                {t('form.feeRate.options.normal', { rate: feeRates.normal })}
              </option>
              <option value={feeRates.fast}>
                {t('form.feeRate.options.fast', { rate: feeRates.fast })}
              </option>
              <option value="custom">{t('form.feeRate.options.custom')}</option>
            </select>
            {feeRate === 'custom' && (
              <input
