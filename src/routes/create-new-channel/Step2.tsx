...
export const Step2 = (props: Props) => {
  ...
  const [maxAssetAmountMap, setMaxAssetAmountMap] = useState<Record<string, number>>({})
  ...
  useEffect(() => {
    const fetchAssetBalances = async () => {
      if (takerAssetsResponse.isSuccess && takerAssetsResponse.data?.assets.length > 0) {
        setAddAsset(true); 
        try {
          const assetPromises = takerAssetsResponse.data.assets.map(asset =>
            assetBalance({ asset_id: asset.asset_id }).then(response => ({ asset_id: asset.asset_id, spendable: response.data?.spendable || 0 }))
          );
          const assetsWithBalances = await Promise.all(assetPromises);
          const newMaxAssetAmountMap = assetsWithBalances.reduce((acc, current) => {
    ...
    fetchAssetBalances();
  }, [takerAssets, assetBalance, takerAssetsResponse.isSuccess, takerAssetsResponse.data?.assets]);
  ...
  const handleCapacityChange = (value: string) => {
    ...
    if (!isNaN(numericValue)) { 
      setValue('capacitySat', Math.min(Math.max(numericValue, MIN_CHANNEL_CAPACITY), maxCapacity), { shouldValidate: true })
    }
  }
  ...
  const onSubmit: SubmitHandler<FormFields> = useCallback(
    (data) => {
      const selectedAsset = takerAssetsResponse.data?.assets.find(asset => asset.asset_id === data.assetId);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="text-center mb-10">
        ...
        <h4 className="text-lg font-medium mb-2">Select the amount of satoshi to spend to open the channel</h4>
      </div>
              ...
              <div className="flex space-x-2">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-red mt-2">
                    {formState.errors.capacitySat && <p className="text-red-500 text-xs italic">{formState.errors.capacitySat.message}</p>}
                  </div>
                </div>
              </div>
            </div>
          { addAsset && (takerAssetsResponse.data?.assets?.length ?? 0) > 0 && (  // Ensure there are assets before showing the option
            <>
                ...
                <div className="flex-1">
                  <input
                    {...register('assetAmount', {
                      min: 0,
                      max: (maxAssetAmountMap as Record<string, number>)[watch('assetId')] || 0,
                      valueAsNumber: true,
                    })}
                    className="rounded bg-blue-dark px-4 py-3 w-full outline-none"
                    type="text"
